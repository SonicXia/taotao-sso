...
<script type="text/javascript">
	var REGISTER={
		...
		beforeSubmit:function() {
				//检查用户是否已经被占用
				$.ajax({
	            	url : REGISTER.param.surl + "/user/check/"+escape($("#regName").val())+"/1?r=" + Math.random(),	//浏览器会缓存相同请求url的结果，这里我们不希望浏览器缓存，所以在最后加上一个随机数
		...
		doSubmit:function() {
			$.post("/user/register",$("#personRegForm").serialize(), function(data){	//把表单序列化，提交到指定的接口里
				if(data.status == 200){
					alert('用户注册成功，请登录！');
					REGISTER.login();
				} else {
